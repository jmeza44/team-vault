...
export const useAnalytics = (
  ...
  const [credentialStats, setCredentialStats] =
    useState<CredentialStatistics | null>(null);
  const [teamActivity, setTeamActivity] = useState<TeamActivityMetrics | null>(
    null
  );
  ...
  const refreshDashboard = useCallback(
    async (currentFilters?: AnalyticsFilters) => {
  ...
  const refreshCredentials = useCallback(
    async (currentFilters?: AnalyticsFilters) => {
      ...
      try {
        const data = await analyticsService.getCredentialStatistics(
          currentFilters || filters
        );
  ...
  const refreshTeamActivity = useCallback(
    async (currentFilters?: AnalyticsFilters) => {
      setIsTeamActivityLoading(true);
      try {
        const data = await analyticsService.getTeamActivityMetrics(
          currentFilters || filters
        );
  ...
  const refreshSecurity = useCallback(
    async (currentFilters?: AnalyticsFilters) => {
      ...
      try {
        const data = await analyticsService.getSecurityMetrics(
          currentFilters || filters
        );
